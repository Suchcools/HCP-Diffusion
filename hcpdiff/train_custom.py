    ...
    def train_one_step(self, data_list):
        with self.accelerator.accumulate(self.TE_unet):
            for idx, data in enumerate(data_list):
                image = data.pop('img').to(self.device, dtype=self.weight_dtype) # img 40 4 64 64 / mask 64 1 64 64 / prompt 40 77
                ...
                model_pred, target, timesteps = self.forward(latents, prompt_ids, **other_datas) # [40, 4, 64, 64]
